...
public enum IPAddress {
    ...
    public init?(fromString inputString: String) {
        ...
        if fromString.hasPrefix("::ffff:") {
            ...
            fromString = inputString.subString(
                fromIndex: inputString.index(inputString.startIndex, offsetBy: 7)
            ).replacing(searchTerm: ":", replacement: ".")
        }

        if fromString.contains(character: ":") {
            // IPv6
            var components: [String]
            if fromString.hasPrefix("::") {
                components = fromString.subString(
                    fromIndex: fromString.index(fromString.startIndex, offsetBy:1)
                ).split(character: ":")
            } else {
                components = fromString.split(character: ":")
            ...
            // IPv4
            let components = fromString.split(character: ".")
...
extension IPAddress: CustomStringConvertible {
    public var description: String {
        ...
        case .IPv6(let s1, let s2, let s3, let s4, let s5, let s6, let s7, let s8):
            ...
            if !result.hasPrefix("::") {
                result = result.subString(fromIndex: result.index(result.startIndex, offsetBy: 1))
            }
            return result
        case .Wildcard:
            return "*"
