...
class DateTests: XCTestCase {
    ...
    func testISODateParsing() {
        if let date = atfoundation.Date(isoDateString: "1984-01-24T12:34:00Z") {
    ...
    func testTimeStampToISODate() {
        let date = atfoundation.Date(timestamp: 443795640)
        XCTAssertNotNil(date.isoDateString)
        XCTAssert(date.isoDateString == "1984-01-24T12:34:00Z")
    }

    func testRFC822DateParsing() {
        if let date = atfoundation.Date(rfc822DateString: "Tue, 24 Jan 1984 12:34:00 +0000") {
    ...
    func testEqualDates() {
        let date1 = atfoundation.Date(timestamp: 443795640)
        let date2 = atfoundation.Date(isoDateString: "1984-01-24T12:34:00Z")!
        XCTAssert(date1 == date2)
    }

    func testBiggerDate() {
        let date1 = atfoundation.Date(timestamp: 443795640)
        let date2 = atfoundation.Date(isoDateString: "1984-01-25T12:34:00Z")!
        XCTAssert(date1 < date2)
    ...
    func testAddingDates() {
        ...
        XCTAssert(date1 - date2 == atfoundation.Date(timestamp: 0))
    }

    func testAddingInterval() {
        let date1 = Date(timestamp: 86400)
        XCTAssert(date1 + TimeInterval(seconds: 100) == atfoundation.Date(timestamp: 86500))
        XCTAssert(date1 + TimeInterval(seconds: -100) == atfoundation.Date(timestamp: 86300))
    }
